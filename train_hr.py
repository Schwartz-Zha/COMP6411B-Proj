...
def train(config):
    ...
    if config.dataset == "tinyImageNet":
        ...
        trainset = cifar10(root='./', train=True, download=True, transform=transform_train, lr_cls=True)

        testset = cifar10(root='./', train=False, download=True, transform=transform_test, lr_cls=True)
    ...
    if config.net == 'vgg16':
        model = vgg16_bn(num_classes=num_classes,dataset=config.dataset)
    ...
    for epoch in range(config.num_epoch):
        ...
        for data, data_hr, lbl in tqdm(trainloader):
            optimizer.zero_grad()
            data_hr, lbl = data_hr.to(config.device), lbl.to(config.device)
            output = model(data_hr)
        ...
        with torch.no_grad():
            for test_data, test_data_hr, test_lbl in testloader:
                test_output = model(test_data_hr.to(config.device))
        ...
        if accuracy >= best_accuracy:
            ...
            if not os.path.exists(f'./model_hr_{config.dataset}'):
                os.mkdir(f'./model_hr_{config.dataset}')
            
            torch.save(model.state_dict(), f'./model_hr_{config.dataset}/{config.net}_best.pth')
    ...
    torch.save(model.state_dict(), f'./model_hr_{config.dataset}/{config.net}_{epoch+1}_{accuracy}.pth')
    ...
    plt.title('accuracy')
    plt.savefig(f'./model_hr_{config.dataset}/loss_acc.png')
