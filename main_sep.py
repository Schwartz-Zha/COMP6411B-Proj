...
def train(config):
    ...
    optimizer = torch.optim.SGD(model_cls.parameters(), lr=config.lr, momentum=0.9, weight_decay=5e-4)
    ...
    for epoch in range(config.num_epoch):
        ...
        for data, data_hr, lbl in tqdm(trainloader):
            ...
            # sr_output = torch.clamp(sr_output, min=-1, max=1)
            cls_output = model_cls(sr_output.detach())
            loss = criterion(cls_output, lbl)
            loss.backward()
        ...
        with torch.no_grad():
            for test_data, test_data_hr, test_lbl in testloader:
                ...
                # test_sr_output = torch.clamp(test_sr_output, min=-1, max=1)
                test_cls_output = model_cls(test_sr_output.detach())
                test_loss = criterion(test_cls_output, test_lbl.to(config.device))
        ...
        if accuracy >= best_accuracy:
            ...
            if not os.path.exists(f'./model_srCls_{config.dataset}_sepTraining'):
                os.mkdir(f'./model_srCls_{config.dataset}_sepTraining')
            
            torch.save(model_cls.state_dict(), f'./model_srCls_{config.dataset}_sepTraining/{config.net}.pth')
    ...
    torch.save(model_cls.state_dict(), f'./model_srCls_{config.dataset}_sepTraining/{config.net}_{epoch+1}_{accuracy}.pth')
    ...
    plt.title('accuracy')
    plt.savefig(f'./model_srCls_{config.dataset}_sepTraining/loss_acc.png')
