...
class cifar10(torchvision.datasets.VisionDataset):
    ...
    def __getitem__(self, index: int) -> Tuple[Any, Any]:
        ...
        """
        hr_img, target = self.data[index], self.targets[index]

        # doing this so that it is consistent with all other datasets
        # to return a PIL Image
        hr_img = Image.fromarray(hr_img)

        lr_img = hr_img.resize((hr_img.width//self.ratio, hr_img.height//self.ratio)) # lr_img
